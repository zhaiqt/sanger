...
parser.add_argument ('-d',"--directory",type=str, help="input files directory", default='/home/zhaiqi1/NGS/mycode/Ab_NGS_4/test2',action='store')
parser.add_argument ('-o',"--outputpath",type=str, help="input files directory", default='/home/zhaiqi1/NGS/mycode/Ab_NGS_4/test2/results/')
...
while not end_of_file_flag:
	...
	for pair in pairDict:
		...
		for element in pair:	
			#Trim from 3 end###
			fastq=TrimEnds.TrimEnds(element)  #apply TrimEnds class
		...
		objectFastA=Merge2FastQ.Merge2FastQ(trimedQ2) #apply Merge2FastQ class
		fasta=objectFastA.merge2FastQ()
		fasta[1]=fasta[1][8:-43]

		if not fasta[1]:
			emptyCounts +=1
		elif len(fasta[1]) < 330:
			continue
		else:
			protein=translator.choose_translation(fasta[1])
			IDCounts +=1	# counts if sequence suvived
...
print "There are %d FastQ seqs that contains >110 amino acids." % IDCounts 
